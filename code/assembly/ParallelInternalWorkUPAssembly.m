...
function str              =  ParallelInternalWorkUPAssembly(str)    
%--------------------------------------------------------------------------
...
parfor ielem=1:str.mesh.volume.n_elem  
    ...
    %----------------------------------------------------------------------
    element_assembly      =  ParallelInternalWorkResidualStiffnessUP(ielem,str.quadrature,str.solution,...
                                      str.geometry,str.mesh,str.fem,...
                                      str.vectorisation,str.material_information);    
    %----------------------------------------------------------------------
    % Sparse assembly
    %----------------------------------------------------------------------
    [INDEXI,INDEXJ,DATA]  =  StiffnessSparseAssemblyInternalWorkUP(ielem,str.geometry.dim,str.mesh,...
                                                    element_assembly);         
    ...
    %----------------------------------------------------------------------
    [INDEXI,INDEXJ,DATA]  =  ParallelInternalVectorsAssemblyUP(ielem,str.geometry.dim,str.mesh,element_assembly);
