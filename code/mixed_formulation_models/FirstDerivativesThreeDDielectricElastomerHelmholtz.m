...
mat_info.derivatives.DPsi.DPsiDH   =  mu2*H - (1/e1)*MatrixScalarMultiplication(dim,dim,ngauss,HE0E0,invJ);
mat_info.derivatives.DPsi.DPsiDE0  =  -(1/e1)*VectorScalarMultiplication(dim,ngauss,HTHE0,invJ);
mat_info.derivatives.DPsi.DPsiDJ   =  -(mu1 + 2*mu2)*invJ + kappa*(J-1) + 1/(2*e1)*((invJ.^2).*HE0HE0) + twoD_Jterm;
