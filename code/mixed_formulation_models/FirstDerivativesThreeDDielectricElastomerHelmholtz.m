function material_information  =  FirstDerivativesThreeDDielectricElastomerHelmholtz(ielem,n_gauss,...
...
for igauss=1:n_gauss
    ...
    material_information.derivatives.DPsi.DPsiDH(:,:,igauss)   =  mu2*H(:,:,igauss) - 1/(e1*J(igauss))*((HE0)*E0(:,igauss)');
    material_information.derivatives.DPsi.DPsiDJ(:,:,igauss)   =  -(mu1 + 2*mu2)/J(igauss) + kappa*(J(igauss)-1) + 1/(2*e1*J(igauss)^2)*(HE0'*HE0);
    material_information.derivatives.DPsi.DPsiDE0(:,igauss)    =  -1/(2*e1*J(igauss))*H(:,:,igauss)'*HE0;
end
